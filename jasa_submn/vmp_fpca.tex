...
accuracy and speed of the VMP algorithm and an application to United States temperature data.
...
implementations of FPCA. VMP represents a class of methodologies, derived from MFVB algorithms,
...
\noindent As stated in \citeA{rohde16}, this alternate expression has the advantage of representing the
...
density function of $\btheta_i$.

The parameter vectors that define each of the optimal $q$-density
functions in \eqref{opt_q} are interrelated and are updated
...
edge with a factor and vice versa. Notice that the edges of the factor graph respect the distributional
...
\begin{enumerate}
	...
	%
	\item Update the messages passed from the factor's neighbouring stochastic nodes to the factor via \eqref{msg_sn_fact}.
	%
	\item Update the messages passed from the factor to its neighbouring stochastic nodes via \eqref{msg_fact_sn}.
	%
\end{enumerate}
...
du$, and $\ind (\cdot)$ is the indicator function.
The exponential family representation of the inverse-$\chi^2$ density function is
...
The multivariate generalization of the inverse-$\chi^2$ distribution is the inverse G-Wishart distribution,
written $\bX \sim \igw (G, \xi, \Lambda)$,
...
inverse-$\chi^2$ random variables. We follow the advice in \citeA[Sections~5 \& 6]{maestrini20} for message passing
...
\begin{itemize}
	...
	These include the Gaussian prior fragment, inverse G-Wishart prior fragment and the iterated inverse
	...
	%
	\item \citeA{maestrini20} provided corrections for the matrix versions of the inverse G-Wishart prior fragment
	and the iterated inverse G-Wishart fragment from \citeA{wand17}. In particular, Section 4.1.3 of
	\citeA{wand17}, which introduces the iterated inverse G-Wishart fragment, only addresses the case where
	the graph parameter is $G = G_{\text{full}}$. However, it does not account for the case where $G = G_{\text{diag}}$.
	The necessary corrections are outlined in Section 6.1 of \citeA{maestrini20}. Although the scalar versions of these
...
\[
	\sum_{i=1}^n \left|\left| y_i - \mu - \sum_{l=1}^L \langle y_i - \mu , \psi_l \rangle \psi_l \right|\right|^2
\]
...
\begin{assumption}
	...
	All eigenvalues of the covariance operator in \eqref{cov_op} have multiplicity one.
...
\begin{theorem}
	...
	curves $y_1, \dots, y_n$ in \eqref{yhat_not_orthogonal}. Then, there exists a unique set of orthonormal
	eigenfunctions $\psi^*_1, \dots, \psi^*_L$ and an uncorrelated set of scores $\zeta^*_{i1}, \dots, \zeta^*_{iL}$,
	$i = 1, \dots, n$, such that
	
	\[
		\yhat_i (t) = \mu (t) + \sum_{l=1}^L \zeta^*_{il} \ \psi^*_l (t), \quad i = 1, \dots, n.
	\]
...
and subsequently estimates the scores given the eigenfunctions using the Karhunen-Lo\`{e}ve decomposition
...
Next, the Bayesian model is described in Section \ref{sec:bayes_mod}, while new fragments that are relevant
for FPCA via VMP are introduced in Sections \ref{sec:fpca_gauss_lik_frag} and \ref{sec:mean_fpc_gauss_pen_frag}.
...
\begin{equation}
	\bzetatilde_i \equiv \T{(1, \T{\bzeta_i})}, \quad i = 1, \dots, n.
\label{zeta_tilde}
\end{equation}
...
a graph message is also required to specify whether the density function respects a full or a diagonal matrix
(see Section \ref{sec:exp_fam_form}). This
...
a storage-economical vech-based representation. The reason for this is outlined in the discussion following
\eqref{zeta_tilde}.
...
\subsection{Establishing a Multivariate Version of the Karhunen-Lo\`{e}ve Decomposition}
...
\noindent Set $\bm_\zeta$ to be the $L \times 1$ sample mean vector of the column vectors of
...
in Lemma \ref{lem:response_est} is a multivariate version of the Karhunen-Lo\`{e}ve decomposition. Here, we define
...
\begin{remark}
	...
	Therefore, \eqref{post_curve_est} may be interpreted as a multivariate version of the truncated
...
\halign{#\cr
	\hbox{%
		\begin{subfigure}{\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/response_curves.pdf}
		\caption{}
	...
	\hbox{%
		\begin{subfigure}{\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/scores.pdf}
		\caption{}
...
\halign{#\cr
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[width=\textwidth]{images/box_plot_sims.pdf}
		\caption{}
	...
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[width=\textwidth]{images/panel_plot.pdf}
		\caption{}
...
\halign{#\cr
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[height=2.5in, width=6.5in]{images/us_fits.pdf}
		\caption{}
	...
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[height=5in, width=6.5in]{images/us_bf.pdf}
		\caption{}
