...
box_plot_fpca_sims <- function(
	...
	N_labels <- rep(N_labels, length(N_vals))
	...
	if(save_pdf) {
		...
		if(logistic_mod) {
			...
			pdf("/Users/tuinolan/Desktop/fulbright/images/logistic_box_plot_sims.pdf",width=plot_width, height=plot_height)
		} else {
			
			pdf("/Users/tuinolan/Desktop/fulbright/images/box_plot_sims.pdf",width=plot_width, height=plot_height)
		}
	}
	...
	box_plots <- bwplot(
		...
		layout=c(3,1),
		xlab="number of response curves",ylab="integrated squared error",
		strip=strip.math,
		par.settings = list(layout.heights = list(strip = 1.2)),
		as.table=TRUE,
		panel=function(...) {
...
panel_plots <- function(
	...
	if(save_pdf) {
		...
		if(logistic_mod) {
			...
			pdf("/Users/tuinolan/Desktop/fulbright/images/logistic_panel_plot.pdf", width=plot_width, height=plot_height)
		} else {
			
			pdf("/Users/tuinolan/Desktop/fulbright/images/panel_plot.pdf", width=plot_width, height=plot_height)
		}
	}
...
# panel_plots <- function(
	# file_names, mu_func, Psi_func, plot_dim,
	# save_pdf=FALSE, logistic_mod=FALSE
# ) {
	...
	# mu_res <- read.table(file_names[1], header=TRUE)
	# mu_res <- as.matrix(mu_res)
	
	# psi_res <- vector("list", length=L)
	# for(l in 1:L) {
		
		# psi_res[[l]] <- read.table(file_names[l+1], header=TRUE)
		# psi_res[[l]] <- as.matrix(psi_res[[l]])
	# }
	...
	# gbl_res <- vector("list", length = L+1)
	# gbl_res[[1]] <- mu_res
	# for(l in 1:L) {
		
		# gbl_res[[l+1]] <- psi_res[[l]]
	# }
	...
	# if(save_pdf) {
		...
		# if(logistic_mod) {
			...
			# pdf("/Users/tuinolan/Desktop/fulbright/images/logistic_panel_plot.pdf",width=10, height=6)
		# } else {
			
			# pdf("/Users/tuinolan/Desktop/fulbright/images/panel_plot.pdf",width=10, height=6)
		# }
	# }
