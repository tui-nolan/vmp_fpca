...
files <- list.files(
	"./us_temp_data_old", recursive = TRUE,
...
n_sample <- 5                             # number of curves for the plots
N_sample <- sort(sample(which(M == max(M)), n_sample)) # specific curves for the plots
...
L_2_plots <- 2                            # number of level 2 eigenfunctions for plots
...
Psi_1_hat[, 1] <- sign(Psi_1_hat[, 1][1])*Psi_1_hat[, 1]
Psi_1_hat[, 2] <- sign(Psi_1_hat[, 2][1])*Psi_1_hat[, 2]
...
for(l in 1:L_1_plots) {
	...
	psi_1_id[l] <- parse(text=paste0("psi[1", l, "] (t) : ", round(var_cont_1[l]), "*\'%\'"))
...
Psi_2_hat[, 1] <- sign(Psi_2_hat[, 1][1])*Psi_2_hat[, 1]
Psi_2_hat[, 2] <- sign(Psi_2_hat[, 2][1])*Psi_2_hat[, 2]
...
for(l in 1:L_2_plots) {
	...
	psi_2_id[l] <- parse(text=paste0("psi[1", l, "] (t) : ", round(var_cont_2[l]), "*\'%\'"))
...
# for(i in 1:n_sample) {
	...
	# N_i <- N_sample[i]
	# lines(zeta_1_summary[[i]][[2]], col = vmp_col)
# }
...
zeta_21_hat <- lapply(Zeta_2_hat[N_sample], function(X) X[, 1])
years <- 1960:1994
