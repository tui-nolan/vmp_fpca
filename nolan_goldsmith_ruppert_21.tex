...
\citeA{goldsmith13} noted that these approaches implicitly condition on estimated eigenbasis to estimate scores, meaning that inference on
individual curve estimates can be poor.

Meanwhile, other approaches have built on or are similar to the probabilistic PCA framework that was introduced by \citeA{tipping99} and \citeA{bishop99}. Rather than first obtaining eigenfunctions from a covariance and then estimating scores, all quantities are considered unknown and are estimated jointly.  \citeA{james2000} used an EM algorithm for estimation and inference in the context of sparsely observed curves. Variational Bayes for FPCA was introduced by \citeA{vanderlinde08} via a generative model with a factorized approximation of the full posterior density function. \citeA{Goldsmith15} introduced a fully Bayes framework for multilevel function-on-scalar
...
\begin{theorem}
	...
	eigenfunctions $\psihat_1, \dots, \psihat_L$ and an uncorrelated set of scores $\zetahat_{i1}, \dots, \zetahat_{iL}$,
...
The Bayesian model is described in Section \ref{sec:bayes_mod}, while new fragments that are relevant
...
\noindent and the functional principal component eigenfunctions were
...
\halign{#\cr
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[height=9cm, width=\textwidth]{images/us_fits.pdf}
		\caption{}
	...
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[height=9cm, width=\textwidth]{images/us_mean_shift.pdf}
		\caption{}
...
\caption{
	...
	perturbations from each functional principal component eigenfunction: $\muhat (t) \pm \delta \psihat_l (t)$,
	$l = 1, \dots, 4$.
}
...
the effect of perturbing the estimated mean function with each eigenfunction: $\muhat (t) \pm \delta \psihat_l (t)$,
...
The first functional principal component eigenfunction, which accounts for 99.1\% of the total variation, is a mean shift
...
The second functional principal component eigenfunction, which accounts 7.2\% of the total variation, shifts
...
between Winter and Summer temperatures will have a strong and positive score for this eigenfunction.
...
The scores for the first eigenfunction indicates that the
lowest temperatures are to be expected in Connecticut and North Dakota, whereas Texas and Kansas have
warmer temperatures. Furthermore, the scores for the second eigenfunction shows that the
...
On the other hand, the contribution of the second eigenfunction to the Kansas weather station
is very weak.
...
algorithm depending on the number of response curves. An application to a large US weather dataset
