...
The $\vect$ and $\vech$ operators are well-established \cite<e.g.>{gentle07}, whereas for a $d^2 \times 1$ vector
...
\begin{itemize}
	...
	%
	\item \citeA{maestrini20} provides corrections for the matrix versions of the Inverse Wishart prior fragment
...
orthonormal. We can establish uncorrelated scores and orthogonal basis functions through an appropriate
...
uncorrelated scores. Ther existence and uniqueness of the basis functions are justified by Theorem \ref{thm:orth_basis}.
...
The error scores in Figure \ref{fig:gauss_resp_sim_st} \subref{subfig:bf_accs} reflect excellent results for the
...
\halign{#\cr
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[height=6cm, width=\textwidth]{images/cw_fits.pdf}
		\caption{}
		\label{subfig:cw_fits}
		\end{subfigure}%
	}\cr
	\noalign{\vfill}
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[height=6cm, width=\textwidth]{images/cw_bf.pdf}
		\caption{}
		\label{subfig:cs_bf}
		\end{subfigure}%
	}\cr
	...
	\hbox{%
		\begin{subfigure}{\textwidth}
		\centering
			\includegraphics[height=6cm, width=\textwidth]{images/cw_mean_shift.pdf}
		\caption{}
...
\caption{
	Application of the VMP algorithm for FPCA to the Canadian weather data. The fits in \subref{subfig:cw_fits}
	are for four randomly selected weather stations in the dataset. The plots in \subref{subfig:cs_bf} are the
	estimated functional principal component basis functions with their proportion of the total explained variance
	included in the panel. The plots in \subref{subfig:cw_mean_shift} show the estimated mean function with
...
is available from the \textsf{fda} package in \textsf{R}. This dataset was popularized by \citeA{ramsay05}, and
...
data points for the $i$th weather station. Based on the results of Chapter 8 of \citeA{ramsay05}, we expect to uncover
...
\citeA{ramsay05}. Note that any variations from the analysis of \citeA{ramsay05} can be attributed to:
...
In Figure \ref{fig:can_weather_data} \subref{subfig:cw_fits}, four randomly selected weather
...
Figures \ref{fig:can_weather_data} \subref{subfig:cs_bf} and \subref{subfig:cw_mean_shift} should be analyzed
concurrently. In figure \ref{fig:can_weather_data} \subref{subfig:cs_bf}, we present the first four estimated functional
principal component basis functions, and in figure \ref{fig:can_weather_data} \subref{subfig:cw_mean_shift}, we show
...
weighting most heavily on the winter months and reflecting the conclusion of \citeA{ramsay05} in that Canadian
...
\citeA{ramsay05}, and we defer the reader to this text for an interpretation of these basis functions.
...
\bibliography{/Users/tuinolan/Desktop/latex_central/bibliography}
